...
public class MergeUsersBolt implements IRichBolt 
{
    ...
    public MergeUsersBolt( String viewConfigFile )
    {
    ...
    public void execute( Tuple input ) 
    {
    	try
    	{
    		List<List> merging = (List) input.getValue( 0 ) ;
            for( List pair : merging ) 
            {
                Integer nodePos = find( pair.get( 0 ) ) ;
                if( null != nodePos ) rankings.set( nodePos, pair ) ;
                else rankings.add( pair ) ;

                Collections.sort( rankings, new Comparator<List>() 
                {
                    public int compare( List o1, List o2 ) { return compareLists( o1, o2 ) ; }
                });

                if ( rankings.size() > k ) rankings.subList( k.intValue(), rankings.size() ).clear() ;
            }
